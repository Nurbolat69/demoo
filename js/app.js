...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const naturalRoadControlPoints = [
    ...
    { x: 67, y: 72 },
    { x: 78, y: 78 }, // Station 4: Foreground table with books & awards
    { x: 70, y: 62 },
    { x: 74, y: 56 }, // Station 5: Forest road & wooden bridge
    { x: 80, y: 54 },
    { x: 85, y: 52 }, // Station 6: Spandiyar Kubeyev's family homestead
    { x: 89, y: 49 },
    { x: 93, y: 47 }, // Station 7: Shezhire & ancestral riverbank yurts
    { x: 97, y: 49 }
  ];
  ...
  function openMuseumModal(stationId) {
    ...
    const nextStation = MUSEUM_STATIONS.find(s => s.id === (st.id % MUSEUM_STATIONS.length + 1));
    ...
    if (modalBody) {
      ...
      if (st.id === 5 && photoList.length >= 2) {
              ...
              return `
                <div class="book-card" onclick="window.openLightbox('${p.src}', '${safeCap}')" title="${safeCap}">
                  <div class="book-cover-wrap">
                    <img src="${p.src}" alt="${safeCap}" class="book-cover-img" loading="lazy" decoding="async" />
        ...
        `;
      } else if ((st.id === 6 || st.id === 7) && photoList.length >= 2) {
        // Station 6 (Отбасы) & Station 7 (Шежіре) Special Grid Showcase
        photoHtml = `
          <div class="modal-gallery-grid station6-grid">
            ${photoList.map(p => {
              ...
              const safeCap = cap.replace(/"/g, '&quot;').replace(/'/g, '&#39;');
              const fitClass = p.type === 'document' ? 'contain-fit' : (p.type === 'portrait' ? 'contain-fit' : 'cover-fit');
              return `
                <div class="gallery-photo-card" onclick="window.openLightbox('${p.src}', '${safeCap}')" title="${safeCap}">
                  <div class="modal-photo-frame portrait-card ${fitClass}">
                    <img src="${p.src}" alt="${safeCap}" class="modal-photo-img ${fitClass}" loading="lazy" decoding="async" />
        ...
        `;
      } else if ((st.id === 6 || st.id === 7) && photoList.length === 1) {
        // Document showcase fallback
        const p = photoList[0];
        const cap = currentLang === 'kz' ? p.captionKaz : p.captionRu;
        const safeCap = cap.replace(/"/g, '&quot;').replace(/'/g, '&#39;');
        photoHtml = `
          <div class="document-showcase" onclick="window.openLightbox('${p.src}', '${safeCap}')" title="${safeCap}">
            <div class="document-frame">
              <img src="${p.src}" alt="${safeCap}" class="document-img" loading="lazy" decoding="async" />
              <div class="zoom-badge">🔍 ${currentLang === 'kz' ? 'Толық экранда көру' : 'Смотреть во весь экран'}</div>
            </div>
            <div class="document-caption">
              <span class="photo-badge gold">📜 ${currentLang === 'kz' ? 'Тарихи құжат' : 'Исторический документ'}</span>
              <span>${cap}</span>
            </div>
          </div>
        `;
        ...
        photoHtml = `
          <div class="modal-gallery-grid">
            ${photoList.map(p => {
              const cap = currentLang === 'kz' ? p.captionKaz : p.captionRu;
              const badge = currentLang === 'kz' ? (p.badgeKaz || '🏛️ Архив құжаты') : (p.badgeRu || '🏛️ Архивный документ');
              const safeCap = cap.replace(/"/g, '&quot;').replace(/'/g, '&#39;');
              return `
                <div class="gallery-photo-card" onclick="window.openLightbox('${p.src}', '${safeCap}')" title="${safeCap}">
                  <div class="modal-photo-frame">
                    <img src="${p.src}" alt="${safeCap}" class="modal-photo-img" loading="lazy" decoding="async" />
                    <div class="zoom-badge">🔍 ${currentLang === 'kz' ? 'Үлкейту' : 'Увеличить'}</div>
                  </div>
                  <div class="modal-photo-caption">
